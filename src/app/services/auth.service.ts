...
export class AuthService {
  ...
  private REGISTER_URL = 'http://localhost:3000/api';
  private tokenKey = 'authToken';

  private REFRESH_URL = 'http://localhost:3000/api';
  ...
  register(name: string, email: string, password: string, passwordConfirmation: string): Observable<any> {
    return this.httpClient.post<any>(this.REGISTER_URL, { name, email, password, password_confirmation: passwordConfirmation }).pipe(
      tap(response => {
        if (response.token) {
          console.log(response.token);
          this.setToken(response.token);
